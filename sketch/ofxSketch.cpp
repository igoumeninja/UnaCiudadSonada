...
void ofxSketch::init(int sketchID, float elast, float aposv) {
...
//######## draw Mouse  ##############//
void ofxSketch::drawMouse(float xL, float yL, float zL, int redL, int greenL, int blueL, int alphaL, float lines) {
	ofNoFill();
...
//########## draw Sound ######################//
void ofxSketch::drawSound(float xL, float yL, float zL, int redL, int greenL, int blueL, int alphaL, float slines) {
	ofFill();
	ofSetColor(redL, greenL, blueL, alphaL);
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
//######## Stop Sound  #############//
void ofxSketch::stoping(float xL, float yL) {
	ofxOscMessage m;
	m.setAddress( "/ampXar" );
	m.addFloatArg( 0);
...
//##########  Sound  #################//
void ofxSketch::sound(float xL, float yL) {
	for (int i=0; i<stoixeia; i++){
		if (i==0){
			deltaX[i] = (xL - xi[i]);
			deltaY[i] = (yL - yi[i]);
