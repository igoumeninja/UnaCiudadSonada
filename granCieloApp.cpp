...
void granCieloApp::setup(){
		...
		}
	{
		...
		rm24.allocateForNScreens(8, 500, 500); //the first dedicate the screens
	...
	}	//	FBO
	{
		...
		casaRedondeada.loadImage("images/casaRedondeada.jpg");
		campanario.loadImage("images/campanario.jpg");
		casaAqua.loadImage("images/casaAqua.jpg");
		casaDerecha.loadImage("images/casaDerecha.jpg");
...
void granCieloApp::update(){ 
	...
	while( receiver.hasWaitingMessages() )
	{
		...
		{		
			...
			if ( m.getAddress() == "/viewCasaDerechaNuevaFALSE" )	{
				viewCasaDerechaNueva = false;
			}					
			if ( m.getAddress() == "/viewGotasTRUE" )	{
				viewGotas = true;
			}		
			if ( m.getAddress() == "/viewGotasFALSE" )	{
				viewGotas = false;
			}		
			if ( m.getAddress() == "/viewCasasNegrasTRUE" )	{
				viewCasasNegras = true;
			}			
			if ( m.getAddress() == "/viewCasasNegrasFALSE" )	{
				viewCasasNegras = false;
			}						
			if ( m.getAddress() == "/viewCasaDerechaTRUE" )	{
				viewCasaDerecha = true;
			}			
			if ( m.getAddress() == "/viewCasaDerechaFALSE" )	{
				viewCasaDerecha = false;
			}						
			if ( m.getAddress() == "/viewCasaAquaTRUE" )	{
				viewCasaAqua = true;
			}			
			if ( m.getAddress() == "/viewCasaAquaFALSE" )	{
				viewCasaAqua = false;
			}						
			if ( m.getAddress() == "/viewCampanarioTRUE" )	{
				viewCampanario = true;
			}			
			if ( m.getAddress() == "/viewCampanarioFALSE" )	{
				viewCampanario = false;
			}						
			if ( m.getAddress() == "/viewCasaGrandeTRUE" )	{
				viewCasaGrande = true;
			}			
			if ( m.getAddress() == "/viewCasaGrandeFALSE" )	{
				viewCasaGrande = false;
			}			
			if ( m.getAddress() == "/viewMuroCasaIzquierdaTRUE" )	{
				viewMuroCasaIzquierda = true;
			}			
			if ( m.getAddress() == "/viewMuroCasaIzquierdaFALSE" )	{
				viewMuroCasaIzquierda = false;
			}			
			if ( m.getAddress() == "/viewTejadoCasaIzquierdaTRUE" )	{
				viewTejadoCasaIzquierda = true;
			}			
			if ( m.getAddress() == "/viewTejadoCasaIzquierdaFALSE" )	{
				viewTejadoCasaIzquierda = false;
			}			
			...
			if ( m.getAddress() == "/viewFlores23TRUE" )	{
				viewFlores23 = true;
			}			
			if ( m.getAddress() == "/viewFlores23FALSE" )	{
				viewFlores23 = false;
			}			
			if ( m.getAddress() == "/viewFlores21TRUE" )	{
				viewFlores21 = true;
			}			
			if ( m.getAddress() == "/viewFlores21FALSE" )	{
				viewFlores21 = false;
			}			
			...
			if ( fbo9 )	{
				...
				if	(	viewSketchCasas	)	{
					...
					for( int i=500; i<600; i++ ) {
						sketch[i].drawMouse(ofRandom(0,500), ofRandom(0,500), 0, 155, 155, 155, 150, 1);	
					}
				}
				if	(viewCasaGrande)	{					
					ofFill();
					ofSetColor(0xFFFFFF);
					casaRedondeada.draw(0,0);						
				}				
				if	(viewCasasNegras)	{					
					ofFill();
					ofSetColor(0,0,0);
			...
			if ( fbo10 )	{
				...
				if	(	viewSketchCasas	)	{
					...
					for( int i=200; i<300; i++ ) {
						sketch[i].drawMouse(ofRandom(0,500), ofRandom(0,500), 0, 205, 25, 155, 155, 1);	
					}
			...
			if ( fbo11) {
				...
				if	(	viewSketchCasas	)	{
					...
					for( int i=300; i<400; i++ ) {
						sketch[i].drawMouse(ofRandom(0,500), ofRandom(0,500), 0, 55, 55, 255, 155, 1);	
					}
			...
			if ( fbo12 )	{
				...
				if	(	viewSketchCasas	)	{
					...
					for( int i=400; i<500; i++ ) {
						sketch[i].drawMouse(ofRandom(0,500), ofRandom(0,500), 0, 255, 15, 55, 155, 1);	
					}
...
//				}					
		...
		}	//	FBO 20
		{
			if ( fbo21) {
				...
				if	( viewCasaDerechaNueva )	{
					ofFill();
					ofSetColor(0xFFFFFF);
					muroCasaDerechaNueva.draw(0,0);						
				}					
		...
		}	//	FBO 22
		{
			if ( fbo23 )	{
				...
				if	( viewCasaDerechaNueva )	{
					ofFill();
					ofSetColor(0xFFFFFF);
					tejadoCasaDerechaNueva.draw(0,0);						
				}									
		...
		}	//	FBO 23
		{
			if ( fbo24 )	{
				...
				if	(	defaultFBO	)	{							
					ofSetColor(255,255,255);
					ofRect(0, 0, rm24.width, rm24.height);							
					ofSetColor(0xFF0000);	
					ofLine(0, 0, rm24.width, rm24.height);							
					ofSetColor(0xFF0000);
					ofLine(rm24.width, 0, 0, rm24.height);							
				}	
				if (viewGotas) {
					glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); // GL_SRC_ALPHA_SATURATE,GL_ONE     GL_SRC_ALPHA, GL_ONE
					ofFill();
					ofSetColor(0xFFFFFF);
					strPosX = ofRandom(0, 500);
					strPosY = ofRandom(0, 500);	
					//cout << "aqui" << endl;
					ofPushMatrix();
					ofTranslate(strPosX, strPosY, 0);
					gotas.draw(0, 0);						
